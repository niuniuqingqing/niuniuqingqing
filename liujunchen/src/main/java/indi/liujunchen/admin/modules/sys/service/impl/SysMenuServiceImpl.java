...
import indi.liujunchen.admin.modules.sys.dao.SysMenuDao;
...
public class SysMenuServiceImpl implements SysMenuService {
	 ...
	 * 获取所有菜单列表
	 */
	private List<SysMenuEntity> getAllMenuListRoot(List<Long> menuIdList){
		//查询根菜单列表
		List<SysMenuEntity> menuList = queryListParentId(0L, menuIdList);
		return menuList;
	}
	...
	@Override
	public List<SysMenuEntity> getUserMenuListDestop(Long userId) {
				//系统管理员，拥有最高权限
				if(userId == Constant.SUPER_ADMIN){
					return getAllMenuListRoot(null);
				}
				//用户菜单列表
				List<Long> menuIdList = sysUserService.queryAllMenuId(userId);
				return getAllMenuListRoot(menuIdList);
	}
}
