// baseURL = ../../
//classroomInquiry 参数列表
// { id: "6", classroomNum: "1", classroomType: "4", isAppointment: "否", approveStatus: "审核中", shijianStart: "2018-05-02", shijianEnd: "2018-05-17", createBy: "admin" }
$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'classroominquiry/self',
        datatype: "json",
        colModel: [			
...
//安卓端调用--获取应用列表list数据
/*$(function () {
	$.get(baseURL + "classroominquiry/self?page=1&limit=10&order=asc&sidx=", function(r){
		console.log(r.page);
    });
});*/
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {//新增和保存方法
		...
		appointment: function (event) {//提交审核
			var id = getSelectedRow();
			if(id == null){
				return ;
			}
		...
		getInfo: function(id){//通过id获取自身对象
			$.get(baseURL + "classroominquiry/info/"+id, function(r){
				console.log(r.classroomInquiry); // { id: "6", classroomNum: "1", classroomType: "4", isAppointment: "否", approveStatus: "审核中", shijianStart: "2018-05-02", shijianEnd: "2018-05-17", createBy: "admin" }
